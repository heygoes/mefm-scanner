...
#
# v11の変更点：
#   ① nan修正：dropna(how="all") → dropna(subset=["Close"])
#      + 決算プレイ/材料株でNaNチェックを追加
#   ② 決算プレイの「上げすぎ除外」：
#      3日で+5%〜+15%に限定（高値掴み防止）
#      ＝ すでに+20%超も上げた後の追いかけを除外
#
# 毎日スキャンする戦略
# ① MEFM A4（スイング）
# ② 決算プレイ候補（yfinance版）
...
def scan_mefm(raw, nk_c):
    ...
    for ticker in TICKERS:
        ...
        # NaNチェック
        if pd.isna(cur_c) or pd.isna(ma100) or pd.isna(atr_n): continue
...
def scan_earnings_proxy(raw, nk_c):
    ...
    for ticker in TICKERS:
        ...
        # NaNチェック（v11追加）
        if pd.isna(cur_c) or pd.isna(c_3d) or pd.isna(ma100): continue
        if c_3d <= 0 or ma100<=0 or vol_avg<=0: continue
        if cur_c <= ma100: continue
        ret_3d = (cur_c - c_3d) / c_3d
        if pd.isna(ret_3d): continue
        # 上げすぎ除外（v11追加）：+5%〜+15%に限定
        if ret_3d < 0.05: continue
        if ret_3d > 0.15: continue   # ← 高値掴み防止
...
def scan_buyback():
    ...
    try:
        ...
        # v11：取得状況・消却など経過報告を除外
        keywords = ["自己株式の取得に係る","自己株式取得枠","自社株買い","取得開始"]
        exclude  = ["取得状況","消却","立会外","処分","終了"]
...
def scan_52week(raw, nk_c):
    ...
    for ticker in TICKERS:
        ...
        vol_now = float(vol.iloc[i-1])
        vol_avg = float(vol.iloc[i-20:i-1].mean())
        if pd.isna(cur_c) or pd.isna(high_52): continue
...
def scan_material(raw, nk_c):
    ...
    for ticker in TICKERS:
        ...
        if pd.isna(cur_c) or pd.isna(prev_c): continue
...
def build_message(today, market, mefm, earnings, buyback, w52, material):
    ...
    lines = [f"📊 MEFM v11 [{date_str}]", f"相場：{market}", ""]
...
def main():
    today = datetime.now()
    print(f"MEFM v11 全戦略統合スキャナー（バグ修正版）")
